...
export class LightningEffect {
    constructor() {
        ...
        Object.defineProperty(this, "boltMat", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "glowMat", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "flashMat", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "strikeTime", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: -10
        });
        Object.defineProperty(this, "nextAuto", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 1
    ...
    // ---- 稲妻生成 ----
    midpointPath(a, b, displace, levels) {
        let pts = [a, b];
        let disp = displace;
    ...
    strike(targetX, targetY) {
        const positions = [];
        const colors = [];
        const gpos = []; // 発光ボディ(ポイント)
        const gcol = [];
        const COL = [0.85, 0.93, 1.0]; // 白青
        const pushPath = (path, lineB, glowB) => {
            // 芯のライン
            for (let i = 0; i < path.length - 1; i++) {
                positions.push(path[i][0], path[i][1], 0, path[i + 1][0], path[i + 1][1], 0);
                for (let k = 0; k < 2; k++)
                    colors.push(COL[0] * lineB, COL[1] * lineB, COL[2] * lineB);
        ...
        // 本線: 画面上端からターゲットへ
        const start = [targetX + (Math.random() - 0.5) * 0.5, 1.05];
