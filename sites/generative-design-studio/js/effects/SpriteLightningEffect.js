 ...
 */
export class SpriteLightningEffect {
    constructor() {
        ...
        Object.defineProperty(this, "lineMat", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    trigger(cx) {
        ...
        const branch = (x0, y0, ang, length, depth) => {
        ...
        for (let i = 0; i < cols; i++) {
            ...
            if (Math.random() < 0.6)
                branch(colx, this.yTop, Math.PI / 2, 0.05 + Math.random() * 0.08, 1); // 冠(上向き)
        }
        // ハロー（上部の淡い赤い輪）
        for (let i = 0; i < 60; i++) {
