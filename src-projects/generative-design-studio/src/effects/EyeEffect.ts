 ...
 */
export class EyeEffect implements Effect {
  readonly id = 'eye';
  readonly title = 'マウス追従の眼';
  ...
  private grab = new THREE.Vector2(0, 0);
  ...
  frame(timeMs: number, dt: number): void {
    ...
    (u.uPupilColor.value as THREE.Color).set(this.params.pupilColor);
    const shapeMap: Record<string, number> = { round: 0, slit: 1, star: 2, heart: 3, cross: 4 };
    ...
    (u.uIris.value as THREE.Color).setHSL(this.params.hue, 0.7, 0.5);
  ...
  pointer(p: Pointer): void {
    if (p.phase === 'down') {
      ...
      if (this.pressed) {
        this.center.set(
          Math.min(1, Math.max(0, p.x + this.grab.x)),
          Math.min(1, Math.max(0, p.y + this.grab.y)),
        );
      } else {
